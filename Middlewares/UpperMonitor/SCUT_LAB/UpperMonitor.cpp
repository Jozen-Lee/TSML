...
void PARAMETER_MODIFICATION(uint8_t * PARAMETER)
{
  ...
  {
    /* 以下部分用于修改参数内容 */
//    case 0x00:  Rotate_Motor_Ctrl.SpeedPID.Kp=PARAMETER_Change_float(PARAMETER+1);
//          break;
//		case 0x01:  Rotate_Motor_Ctrl.SpeedPID.Ki=PARAMETER_Change_float(PARAMETER+1);
//          break;
//		case 0x02:  Rotate_Motor_Ctrl.SpeedPID.Kd=PARAMETER_Change_float(PARAMETER+1);
//          break;
//		case 0x03:  Rotate_Motor_Ctrl.SpeedPID.Target=PARAMETER_Change_float(PARAMETER+1);
//          break;
//     default:break;
	/* 以上部分用于修改参数内容 */
  }
              ...
              if(USART_receive[0]==0x0a)
              {
                for(int j=1;j<5;j++)
                {
                  if(USART_receive[j]!=0x0a)
					  USART_Interrupt_flag=0xff;    //回到等待模式，当发送的都是0x0a的时候说明稳定不变？？？
                }
                if(USART_Interrupt_flag==0x02)
                {
                  On_Off_flag =1;
                  USART_Interrupt_flag=0xff;    //回到等待模式，emmm可能是结束了？
                }
              }
              else if(USART_receive[0]==0xb0)
              {
                for(int j=1;j<5;j++)
                {
                  if(USART_receive[j]!=0xb0)
                    USART_Interrupt_flag=0xff;   //回到等待模式，这是干啥了？？？
                }
                if(USART_Interrupt_flag==0x02)	//这不本来USART_Interrupt_flag就是0x02?何必多此一举呢
                {
                  On_Off_flag =0;
                  USART_Interrupt_flag=0xff;    //回到等待模式，开始信号？？？
                }
              }
              else 
                USART_Interrupt_flag=0xff;     //回到等待模式
